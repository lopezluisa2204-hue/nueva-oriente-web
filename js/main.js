...
async function loadProperties() {
  ...
  // Blindaje: si por algún error de guardado "fotos" queda como un solo
  // texto en vez de una lista, se normaliza aquí para que la página no se
  // rompa (en vez de mostrar "No se pudo cargar la información").
  return items.map((p) => {
    if (typeof p.fotos === "string") {
      return { ...p, fotos: p.fotos ? [p.fotos] : [] };
    }
...
function renderDetail(properties) {
  ...
  // El resto de fotos, sin repetir las 3 que ya se muestran arriba
  // (portada + las dos de al lado).
  const restantes = fotos.slice(3);
  ...
  const masFotosHtml = restantes.length
    ? `<h2 style="margin-top:32px;">Más fotos</h2>
       <div class="more-photos-grid">
         ${restantes.map((f) => `<div class="more-photo"><img src="${escapeHtml(f)}" alt="${escapeHtml(p.titulo)}"></div>`).join("\n")}
       </div>`
    : "";

  // Estos datos son opcionales: un lote, por ejemplo, no tiene habitaciones,
  // baños, área construida ni pisos. El bloque completo se omite si no hay
  // ninguno diligenciado.
  const specItems = [
    p.habitaciones ? `<div><span class="num">${escapeHtml(p.habitaciones)}</span><span class="label">Habitaciones</span></div>` : "",
    p.banos ? `<div><span class="num">${escapeHtml(p.banos)}</span><span class="label">Baños</span></div>` : "",
    p.area ? `<div><span class="num">${escapeHtml(p.area)}</span><span class="label">Área construida</span></div>` : "",
    p.pisos ? `<div><span class="num">${escapeHtml(p.pisos)}</span><span class="label">Pisos</span></div>` : "",
  ].filter(Boolean);
  const specListHtml = specItems.length
    ? `<div class="spec-list">${specItems.join("\n")}</div>`
    : "";
...
function initLightbox() {
  const imgs = Array.from(document.querySelectorAll(".gallery img, .detail-desc .more-photos-grid img"));
